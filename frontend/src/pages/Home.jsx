...
function Home() {
  ...
  const { Movies, PageDetails } = useSelector((state) => state.movie);
      ...
      <div className="container">
        <div className="flex gap-5 flex-col w-[100%] listing-wrap">
          <div className="flex justify-between head-list">
            ...
            <div
              className="flex flex-row gap-3 items-center log-wrap cursor-pointer"
          ...
          <div className="flex flex-wrap listing-wrapper">
            {Movies.map((val) => (
              <Card
                movieName={val.name}
                image={"http://192.168.1.43:8001/" + val.cover_pic}
                year={val.publishing_year}
                id={val._id}
              />
            ))}
          </div>
